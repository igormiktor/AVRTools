/*
    USART2.h - Functions and classes to use USART2 on AVR systems for
 ...
 * \file
 *
 * \brief This file provides functions that offer high-level interfaces to %USART2 hardware, which is
 ...
 * with %USART2.  This means the transmit functions return immediately after queuing data in the output buffer
 * for transmission and the transmission happens asynchronously, using
 * dedicated %USART2 hardware. Similarly, data is received asynchronously and placed into the input buffer.
 *
 ...
 * define the macros \c USART2_RX_BUFFER_SIZE (for the receive buffer) and \c USART2_TX_BUFFER_SIZE
 * (for the transmit buffer) to whatever sizes you need.  You need to make these define these macros prior
 * to compiling the file USART2.cpp.
 *
 * Two interfaces are provided.  USART2 is a functional interface that makes use of the buffering and
 * asynchronous transmit and receive capabilities of the microcontrollers.  However, USART2 is limited to
 * transmitting and receiving byte and character streams.
 *
 * Serial2 is the most advanced and capable interface to the %USART2 hardware.  Serial2 provides a
 ...
 *
 * \note Linking against USART2.cpp installs interrupt functions for transmit and receive on %USART2
 * (interrupts USART2_UDRE and USART2_RX on
 * Arduino Mega/ATmega2560).  You cannot use the minimal interface to %USART2 (from USARTMinimal.h)
 * if you link against USART2.cpp.  In particular, do \e not call initUSART2() or clearUSART2()
 * if you link against USART2.cpp.
 */
...
/*!
 * \brief This namespace bundles a high-level buffered interface to the %USART2 hardware.  It provides logical cohesion and
...
namespace USART2
{
    ...
    /*!
    * \brief Initialize %USART2 for buffered, asynchronous serial communications using interrupts.
    *
    ...
    */
    ...
    /*!
    * \brief Stops buffered serial communications using interrupts on %USART2.
    *
    * After calling this function, Arduino pins 0 and 1 are released and available for
    * use as ordinary digital pins.
    *
    * If you want to use %USART2 again for buffered, asynchronous serial communications, you must
    * again call start().
    */
    ...
    *
    * This function blocks until the transmit buffer is empty and the last byte has been transmitted by %USART2.
...
/*!
 * \brief Provides a high-end interface to serial communications using %USART2.
 *
 * The functions in this class are buffered for both input and output and operate using interrupts associated
 * with %USART2.  This means the write functions return immediately after queuing data in the output buffer
 * for transmission and the transmission happens asynchronously, using
 * dedicated %USART2 hardware. Similarly, data is received asynchronously and placed into the read buffer.
 *
...
class Serial2 : public Writer, public Reader
{
public:
     ...
     * function before reading from or writing to Serial2 on %USART2.
     *
     * \arg \c baudRate the baud rate for the communications, usually one of the following
     * values: 300, 600, 1200, 2400, 4800, 9600, 14400, 19200, 28800, 38400, 57600, or 115200
     * (although other values below can be specified).
     ...
     */
    void start( unsigned long baudRate, UsartSerialConfiguration config = kSerial_8N1 )
    { USART2::start( baudRate, config ); }


    /*!
     * \brief Stops buffered serial communications using Serial2 on %USART2 by deconfiguring
