/*
    USART1.h - Functions and classes to use USART1 on AVR systems for
 ...
 * \file
 *
 * \brief This file provides functions that offer high-level interfaces to %USART1 hardware, which is
 ...
 * with %USART1.  This means the transmit functions return immediately after queuing data in the output buffer
 * for transmission and the transmission happens asynchronously, using
 * dedicated %USART1 hardware. Similarly, data is received asynchronously and placed into the input buffer.
 *
 ...
 * define the macros \c USART1_RX_BUFFER_SIZE (for the receive buffer) and \c USART1_TX_BUFFER_SIZE
 * (for the transmit buffer) to whatever sizes you need.  You need to make these define these macros prior
 * to compiling the file USART1.cpp.
 *
 * Two interfaces are provided.  USART1 is a functional interface that makes use of the buffering and
 * asynchronous transmit and receive capabilities of the microcontrollers.  However, USART1 is limited to
 ...
 *
 * \note Linking against USART1.cpp installs interrupt functions for transmit and receive on %USART1
 * (interrupts USART1_UDRE and USART1_RX on
 * Arduino Mega/ATmega2560).  You cannot use the minimal interface to %USART1 (from USARTMinimal.h)
 * if you link against USART1.cpp.  In particular, do \e not call initUSART1() or clearUSART1()
...
namespace USART1
{
    ...
    *
    * If you want to use %USART1 again for buffered, asynchronous serial communications, you must
    * again call start().
    */
 ...
 * with %USART1.  This means the write functions return immediately after queuing data in the output buffer
 * for transmission and the transmission happens asynchronously, using
 * dedicated %USART1 hardware. Similarly, data is received asynchronously and placed into the read buffer.
 *
